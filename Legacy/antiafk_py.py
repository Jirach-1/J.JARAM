...
try:
    from antiafk_native import (  # type: ignore
        action_task as _native_action_task,
        clear_notopmost_all_roblox_windows as _native_clear_notopmost_all_roblox_windows,
        find_roblox_windows as _native_find_roblox_windows,
    )

    _USING_NATIVE = True
except Exception:  # pragma: no cover
    _native_action_task = None
    _native_clear_notopmost_all_roblox_windows = None
    _native_find_roblox_windows = None
    _USING_NATIVE = False


def _clear_notopmost(hwnd: int) -> None:
    """Best-effort: ensure a window is NOT TOPMOST (prevents 'always on top' sticking after errors)."""
    try:
        ...
        win32gui.SetWindowPos(
            ...
            0,
            0,
            win32con.SWP_NOMOVE | win32con.SWP_NOSIZE | getattr(win32con, "SWP_NOACTIVATE", 0),
        )
    except Exception:
        pass


def _clear_notopmost_all_roblox_windows() -> int:
    """
    Clears TOPMOST from all RobloxPlayerBeta windows (including hidden ones).
    Returns how many windows were processed.
    """
    if _USING_NATIVE and _native_clear_notopmost_all_roblox_windows:
        try:
            return int(_native_clear_notopmost_all_roblox_windows())
        except Exception:
            pass

    windows: list[int] = []

    def enum_window_callback(hwnd, out):
        try:
            _, process_id = win32process.GetWindowThreadProcessId(hwnd)
            if not process_id:
                return True
            process = psutil.Process(process_id)
            if str(process.name() or "").lower() == "robloxplayerbeta.exe":
                out.append(int(hwnd))
        except Exception:
            pass
        return True
    ...
    for h in windows:
        _clear_notopmost(int(h))

    return len(windows)

# Optional Tk support for legacy hosts.
# JARAM uses this module without Tkinter, so we fall back to a stub
# that still provides tk.TclError for exception handling.
try:
...
def _antiafk_action_task(hwnd: int, base_action: str, menu_autoreconnect: bool, in_menu: bool):
    """
    Run a single Anti-AFK action in a separate process.
    Returns (success, message).
    """
    effective_action = "AutoReconnect" if (menu_autoreconnect and in_menu) else base_action

    try:
        if _USING_NATIVE and _native_action_task:
            try:
                return _native_action_task(int(hwnd), str(base_action), bool(menu_autoreconnect), bool(in_menu))
            except Exception:
                pass

        if not win32gui.IsWindow(hwnd):
            return False, f"Window {hwnd} is not a valid window"

        window_title = win32gui.GetWindowText(hwnd)
        if not window_title or "Roblox" not in window_title or any(x in window_title for x in ["MSCTFIME", "Default IME"]):
            return False, f"Window '{window_title}' is not a valid Roblox window"
        ...
        try:
            ...
            def map_virtual_key(vk_code: int):
            ...
            if effective_action == 'space':
            ...
            elif effective_action == 'AutoReconnect':
                ...
                vk_code_A = ctypes.windll.user32.VkKeyScanA(ord('a')) & 0x41
                scan_code_A = map_virtual_key(vk_code_A)
                ctypes.windll.user32.keybd_event(vk_code_A, scan_code_A, 0, 0)
                time.sleep(ALT_DELAY / 1000.0)
                ctypes.windll.user32.keybd_event(vk_code_A, scan_code_A, win32con.KEYEVENTF_KEYUP, 0)
                time.sleep(ALT_DELAY / 1000.0)
            ...
            if old_hwnd and old_hwnd != hwnd and win32gui.IsWindow(old_hwnd):
                if win32gui.IsWindowVisible(old_hwnd) and not win32gui.IsIconic(old_hwnd):
                    ...
                    try:
                        ...
                        _clear_notopmost(int(old_hwnd))

            return True, f"Performed {effective_action} action on '{window_title}'"
            ...
            try:
                ...
                pass
            return False, f"Error performing anti-AFK action on '{window_title}': {e}"
    except Exception as e:
        return False, f"Anti-AFK sub-process failure for {hwnd}: {e}"
    finally:
        # Never leave Roblox (or the previous foreground window) stuck as TOPMOST.
        try:
            _clear_notopmost(int(hwnd))
        except Exception:
            pass
        try:
            if old_hwnd:
...
class AntiAFK:
    ...
    def __init__(self, parent, config=None):
        ...
        if 'multi_instance_enabled' not in self.config:
            # Always treat multi-instance as enabled in the JARAM host
        ...
        if 'antiafk_menu_autoreconnect' not in self.config:
            self.config['antiafk_menu_autoreconnect'] = False

        self.antiafk_running = False
        self.antiafk_stop_event = threading.Event()
        self.antiafk_pause_event = threading.Event()
        self.antiafk_paused_event = threading.Event()
        ...
        # Optional hooks that let non-Tk hosts mirror state/log updates
        self.status_callback = None
        self.button_state_callback = None
        # Optional host-provided callback: pid -> bool|None (in_menu?)
        self.is_pid_in_menu_callback = None

        if self.config.get('antiafk_user_safe', False):
            self.start_activity_monitor()

    def _cleanup_topmost_after_error(self, *, reason: str = "") -> None:
        """
        If any Roblox window was accidentally left TOPMOST (e.g., an exception between TOPMOST/NOTOPMOST),
        force-clear it so Roblox doesn't stick above other apps.
        """
        try:
            n = _clear_notopmost_all_roblox_windows()
            if reason:
                self.update_status(f"Topmost cleanup ({reason}): processed {n} Roblox window(s).")
        except Exception:
            pass
    ...
    def create_tab(self, notebook):
        ...
        self.menu_autoreconnect_var = tk.BooleanVar(value=self.config.get("antiafk_menu_autoreconnect", False))
        self.menu_autoreconnect_cb = ttk.Checkbutton(
            controls_frame,
            text="Use AutoReconnect when in main menu",
            variable=self.menu_autoreconnect_var,
            command=self.update_config
        )
        self.menu_autoreconnect_cb.grid(row=5, column=0, columnspan=2, sticky="w", padx=5, pady=5)

        sequential_delay_frame = ttk.Frame(controls_frame)
        sequential_delay_frame.grid(row=6, column=0, columnspan=2, sticky="w", padx=25, pady=2)
    ...
    def update_config(self):
        """Update configuration based on current Tk UI settings."""
        try:
            ...
            if hasattr(self, "menu_autoreconnect_var"):
                try:
                    self.config["antiafk_menu_autoreconnect"] = bool(self.menu_autoreconnect_var.get())
                except Exception:
                    self.config["antiafk_menu_autoreconnect"] = False
            ...
            if hasattr(self.parent, "config_changed"):
                self.parent.config_changed = True
            ...
            self.update_status(
                f"Configuration updated: Interval={self.config['antiafk_interval']}s, Action={self.config['antiafk_action']}"
            )
        except Exception as e:
    ...
    def apply_host_config(
        self,
        *,
        multi_instance_enabled=None,
        interval=None,
        action=None,
        user_safe=None,
        sequential_mode=None,
        sequential_delay=None,
        menu_autoreconnect=None,
    ):
        """
        Update configuration when hosted by a non-Tk UI (e.g. PyQt).

        All parameters are optional; None means "leave existing value".
        """
        try:
            ...
            if user_safe is not None:
                self.config["antiafk_user_safe"] = bool(user_safe)

            if sequential_mode is not None:
                self.config["antiafk_sequential_mode"] = bool(sequential_mode)

            if sequential_delay is not None:
                try:
                    delay = float(sequential_delay)
                    if delay < 0.1:
                        delay = 0.1
                    elif delay > 5.0:
                        delay = 5.0
            ...
            self.log_error(e, "Error updating Anti-AFK configuration from host")
    ...
    def update_button_states(self):
        ...
        # Allow an external UI host (e.g., PyQt) to mirror state changes
        if self.button_state_callback:
            try:
                self.button_state_callback(antiafk_enabled)
            except Exception:
                pass
    ...
    def _ensure_action_pool(self, workers: int) -> ProcessPoolExecutor:
        """Lazy-create a process pool for Anti-AFK actions."""
        workers = max(1, workers)
        if self._action_pool is None:
            self._action_pool = ProcessPoolExecutor(max_workers=workers, mp_context=self._mp_context)
        return self._action_pool

    def _run_action_in_process(self, hwnd: int, action_type: str, menu_autoreconnect: bool) -> tuple:
        """
        Run a single Anti-AFK action in a separate process, sequentially.
        Falls back to the in-process implementation if the pool fails.
        """
        try:
            pool = self._ensure_action_pool(1)
        except Exception as e:
            self.update_status(f"Falling back to in-process Anti-AFK (pool error: {e})")
            return self.perform_antiafk_action(hwnd, action_type), ""

        in_menu_flag = False
        if menu_autoreconnect and self.is_pid_in_menu_callback:
            try:
                _, pid = win32process.GetWindowThreadProcessId(hwnd)
                in_menu_flag = bool(self.is_pid_in_menu_callback(int(pid)))
            except Exception:
                in_menu_flag = False

        fut = pool.submit(_antiafk_action_task, int(hwnd), action_type, menu_autoreconnect, in_menu_flag)
        try:
            success, message = fut.result()
        except Exception as e:
            return False, f"Anti-AFK sub-process error: {e}"
        return success, message

    def _shutdown_action_pool(self):
        """Tear down the Anti-AFK process pool cleanly."""
        if self._action_pool:
            try:
    ...
    def update_status(self, message):
        ...
        # External callback first so hosts see updates even without Tk widgets
        ...
        # Legacy Tk UI support (unused in JARAM, but kept for compatibility)
        ...
        try:
            ...
            # Any UI failure here is non-fatal; just ignore it.
            pass
    ...
    def toggle_multi_instance(self):
        ...
        # Legacy Tk UI only. In the JARAM host, multi-instance is always on.
        enabled = True
        self.config['multi_instance_enabled'] = True
        try:
            ...
            self.update_status("Failed to enable multi-instance mutex")
    ...
    def find_roblox_windows(self, include_hidden=True):
        ...
        if _USING_NATIVE and _native_find_roblox_windows:
            try:
                return [int(h) for h in _native_find_roblox_windows(bool(include_hidden))]
            except Exception:
                pass
    ...
    def perform_antiafk_action(self, hwnd, action_type=None):
        ...
        # Base action comes from explicit argument or config
        base_action = action_type or self.config.get('antiafk_action', 'space')

        menu_autoreconnect = bool(self.config.get("antiafk_menu_autoreconnect", False))
        in_menu = False
        if menu_autoreconnect and self.is_pid_in_menu_callback:
            try:
                _, pid = win32process.GetWindowThreadProcessId(hwnd)
                in_menu = bool(self.is_pid_in_menu_callback(int(pid)))
            except Exception:
                in_menu = False

        if _USING_NATIVE and _native_action_task:
            try:
                success, message = _native_action_task(int(hwnd), str(base_action), bool(menu_autoreconnect), bool(in_menu))
                if not success and message:
                    self.update_status(message)
                return bool(success)
            except Exception as e:
                self.update_status(f"Native Anti-AFK failed (falling back to Python): {e}")
        ...
        # Detect per-window main-menu state if host supports it
        effective_action = base_action
        if menu_autoreconnect and in_menu:
            effective_action = 'AutoReconnect'
        ...
        try:
            ...
            # Combined focus strategy:
            # - If minimized, restore like the original script.
            # - Use AttachThreadInput to improve SetForegroundWindow reliability,
            #   but avoid aggressive TOPMOST hacks that caused system beeps.
            if was_minimized:
                win32gui.ShowWindow(hwnd, win32con.SW_RESTORE)
            ...
            try:
                ...
                # Fall back to a simple SetForegroundWindow if anything fails
                try:
                    win32gui.SetForegroundWindow(hwnd)
                except Exception:
                    pass
            ...
            # Safety: ensure we never leave Roblox or the previous foreground window stuck TOPMOST.
            try:
                _clear_notopmost(int(hwnd))
            except Exception:
                pass
    ...
    def start_antiafk(self):
        ...
        self.antiafk_pause_event.clear()
        self.antiafk_paused_event.clear()
    ...
    def stop_antiafk(self):
        ...
        self._shutdown_action_pool()
        self.stop_activity_monitor()

        self.antiafk_running = False
        self.antiafk_thread = None

        # Safety: ensure no Roblox window is left stuck TOPMOST after errors.
        self._cleanup_topmost_after_error(reason="stop")

        self.update_status("Anti-AFK stopped")
        self.update_button_states()

    def pause_antiafk(self, wait=True):
        """Pause the Anti-AFK loop without resetting its timers."""
        if not self.antiafk_running:
            return False

        if not self.antiafk_pause_event.is_set():
            self.antiafk_pause_event.set()

        if wait:
            while self.antiafk_running and not self.antiafk_paused_event.is_set():
                if self.antiafk_stop_event.wait(timeout=0.05):
                    break
        return True

    def resume_antiafk(self):
        """Resume the Anti-AFK loop after a pause."""
        if not self.antiafk_running:
            return False
        self.antiafk_pause_event.clear()
        return True
    ...
    def antiafk_loop(self):
        ...
        try:
            ...
            while not self.antiafk_stop_event.is_set():
                try:
                    if self.antiafk_pause_event.is_set():
                        if pause_started is None:
                            pause_started = time.time()
                            self.antiafk_paused_event.set()
                            self.update_status("Anti-AFK paused")

                        while self.antiafk_pause_event.is_set() and not self.antiafk_stop_event.is_set():
                            self.antiafk_stop_event.wait(timeout=0.2)
                        ...
                        last_status_update_time += paused_for
                        pause_started = None
                        self.antiafk_paused_event.clear()
                        self.update_status("Anti-AFK resumed")

                    current_time = time.time()

                    # Re-read timing/action config each cycle so UI changes apply immediately
                    try:
                        interval = int(self.config.get('antiafk_interval', interval))
                    except Exception:
                        interval = interval
                    action_type = self.config.get('antiafk_action', action_type)
                    user_safe = bool(self.config.get('antiafk_user_safe', user_safe))
                    sequential_mode = bool(self.config.get('antiafk_sequential_mode', sequential_mode))
                    try:
                        sequential_delay = float(self.config.get('antiafk_sequential_delay', sequential_delay))
                    except Exception:
                    ...
                    if perform_action:
                        ...
                        # Always apply Anti-AFK to all detected Roblox windows sequentially.
                        # Multi-instance config now only controls the Roblox mutex, not which
                        # windows receive actions.
                        for i, window in enumerate(roblox_windows):
                            if self.antiafk_stop_event.is_set():
                                break
                            success, message = self._run_action_in_process(window, action_type, menu_autoreconnect)
                            if not success:
                                if message:
                                    self.update_status(message)
                            ...
                            if use_sequential and i < len(roblox_windows) - 1:
                                if self.antiafk_stop_event.wait(timeout=sequential_delay):
                                    break
                        ...
                        if action_success:
                            ...
                            self._cleanup_topmost_after_error(reason="action failed")
                    ...
                    self._cleanup_topmost_after_error(reason="loop error")
                    if self.antiafk_stop_event.wait(timeout=5):
                        break
    ...
    def test_action_with_delay(self):
        ...
        # Disable Tk test button if present (legacy UI)
        ...
        def run_test():
            try:
                # Preserve original behavior: small delay before sending keys
                time.sleep(3.0)
        ...
        # Legacy Tk host: use widget.after; otherwise run in a background thread
        if hasattr(self, "status_text") and hasattr(self.status_text, "after"):
            try:
    ...
    def shutdown(self):
        ...
        try:
            ...
            # Safety: ensure no Roblox window is left stuck TOPMOST after errors.
            self._cleanup_topmost_after_error(reason="shutdown")
        except Exception:
